#!/usr/bin/env python
#
# Release script for wish-compiler
#
...
version = verify(File("project.clj")
                 .filtersTo(RegexFilter('defproject wish-compiler "(.*)"'))
...
binFile = "target/base+system+user+dev/wish-compiler"
verify(Execute('lein bin')).succeeds(silent=False)
...
verify(gitRelease).uploadFile(binFile, 'application/octet-stream')
